  ...
  <div style="margin-top: 10px" class="app-contain">
    ...
    <div style="display: flex;flex-direction: row">
        ...
        <el-card>
          <el-table v-loading="listLoading" :data="tableData" fit highlight-current-row style="width: 100%" @row-click="itemSelect">
            ...
            <el-table-column  align="right" width="70">
              <template slot-scope="{row}">
                <router-link target="_blank" :to="{path:'/read',query:{id:row.id}}" v-if="row.status === 1 && row.paperType==3 ">
                  <el-button  type="text" size="small">待批改</el-button>
                </router-link>
                <!--<router-link target="_blank" :to="{path:'/edit',query:{id:row.id}}" v-else-if="row.status === 1 ">
                  <el-button  type="text" size="small">去批改</el-button>
                </router-link>-->
                <router-link target="_blank" :to="{path:'/read',query:{id:row.id}}" v-else>
      ...
      <div style="width: 300px;">
        <el-card  class="record-answer-info" style="width:300px;">
