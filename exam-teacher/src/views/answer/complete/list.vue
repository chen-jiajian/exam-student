  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="140px" label="操作" align="center">
        <template slot-scope="{row}">
          <router-link target="_blank" :to="{path:'/answer/completeRead', query:{id:row.id}}">
            <el-button size="mini">查看</el-button>
          </router-link>
          <router-link target="_blank" :to="{path:'/answer/print', query:{id:row.id}}" class="link-left">
...
export default {
  ...
  methods: {
    ...
    search () {
      ...
      examPaperAnswerApi.completePageList(this.queryParam).then(data => {
