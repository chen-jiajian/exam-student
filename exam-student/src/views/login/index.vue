        ...
        <div class="lowin-box-inner">
          <el-form ref="loginForm" :model="loginForm" :rules="loginRules">
            ...
            <div class="lowin-group password-group">
              <label>密码 <a href="javascript:void(0);" class="forgot-link">忘记密码?</a></label>
  ...
  methods: {
    ...
    handleLogin () {
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          let encodePassword = rsaEncryptStr(process.env.VUE_APP_PasswordPublicKey, this.loginForm.password)
          let data = { userName: this.loginForm.userName, password: encodePassword, remember: this.loginForm.remember }
          loginApi.login(data).then(function (result) {
            if (result && result.code === 1) {
              _this.setUserName(result.response.userName)
              _this.setImagePath(result.response.imagePath)
    ...
    ...mapMutations('user', ['setUserName', 'setImagePath'])
  }
}
</script>
